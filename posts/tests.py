        ...
        for url in url_list:
            ...
            if response.context:
                if "page" in response.context:
                    ...
                    self.assertEqual(
                        response.context["page"][0].group, self.group
                        )
                    self.assertEqual(
                        response.context["page"][0].author, self.user
                        )
                    self.assertEqual(
                        response.context["page"][0].text, post_text
                        )
                    ...
                    self.assertEqual(
                        response.context["post"].text, post_text
                        )
                    self.assertContains(response, post_text)
            else:
                print("not context")                    
